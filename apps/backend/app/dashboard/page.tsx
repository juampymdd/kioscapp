        ...
        <DashboardTabs
          tabs={[
                ...
                <div className="space-y-6">
                  ...
                  {data.empty ? (
                    <div className="text-center py-16 bg-slate-900 border border-slate-800 rounded-2xl">
                      ...
                      <p className="text-slate-500 text-sm mt-1">Las estadísticas aparecerán cuando el desktop sincronice ventas.</p>
                    </div>
                  ) : (
            ...
            },
            {
              ...
              panel: data.empty ? (
                <div className="text-center py-16 bg-slate-900 border border-slate-800 rounded-2xl">
                  <p className="text-slate-400">Sin datos para analizar todavía.</p>
                  <p className="text-slate-500 text-sm mt-1">Los gráficos aparecen cuando hay ventas sincronizadas.</p>
                </div>
              ) : (
            ...
            {
              id: 'sucursales',
              label: `Sucursales · ${data.sucursales.length}`,
              panel: (
                  ...
                  {data.sucursales.map(suc => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium shrink-0 ${suc.activo ? 'bg-emerald-500/15 text-emerald-300' : 'bg-slate-800 text-slate-400'}`}>
                      ...
                      <div className="border-t border-slate-800 pt-4 flex justify-between items-end">
                        ...
                        <p className="text-slate-400 text-sm">{suc.cajas} caja{suc.cajas !== 1 ? 's' : ''}</p>
                      </div>
                    </Link>
                  ))}
                </div>
